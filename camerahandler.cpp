...
void CameraHandler::reportErrors(QMediaPlayer::Error e)
{
    ...
    case QMediaPlayer::Error::FormatError:
        logger->write("Camera Error: Media format is not (fully) supported! Playback may still be "
                      "possible but features may be missing.");
        break;
...
void CameraHandler::updateWithSettings()
{
    if (vw->parentWidget() != NULL) {
        if (settings->value(SettingsConstants::CONN_CAM_EN, SettingsConstants::D_CONN_CAM_EN)
                .toBool()) {
            vw->show();
        } else {
            vw->hide();
    ...
    QString url = settings
                      ->value(SettingsConstants::CONN_CAM_ADDRESS,
                              SettingsConstants::D_CONN_CAM_ADDRESS)
                      .toString();
    qDebug() << url;
